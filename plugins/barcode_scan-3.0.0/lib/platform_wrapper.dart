...
class BarcodeScanner {
  ...
  static Future<ScanResult> scan({
    ...
    assert(options != null);
    if (Platform.isIOS) {
      return _doScan(options, 'iosScan');
    }
    ...
    subscription = events.listen((event) async {
      if (event is String) {
        if (event == cameraAccessGranted) {
          subscription.cancel();
          completer.complete(await _doScan(options, 'androidScan'));
    ...
    if (permissionsRequested) {
      ...
      subscription.cancel();
      return _doScan(options, 'androidScan');
    }
  }

  static Future<ScanResult> _doScan(ScanOptions options, String method) async {
    var config = proto.Configuration()
          ...
          ..android = (proto.AndroidConfiguration()
              ...
              /**/)
        /**/;
    var buffer = await _channel.invokeMethod(method, config?.writeToBuffer());
