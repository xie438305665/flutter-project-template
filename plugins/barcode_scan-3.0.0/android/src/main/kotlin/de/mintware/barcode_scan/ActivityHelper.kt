...
class ActivityHelper(private var applicationContext: Context?,
    ...
    fun showIosScannerActivity(result: MethodChannel.Result, config: Protos.Configuration) {
        if (activity == null) {
            Log.d(TAG, "Could not launch BarcodeScannerActivity because the plugin is not attached to any activity")
            return
        }

        activityResultMap[REQ_START_SCAN] = IosScanResultHandler(result)
    ...
    fun showAndroidScannerActivity(result: MethodChannel.Result) {
        if (activity == null) {
            Log.d(TAG, "Could not launch BarcodeScannerActivity because the plugin is not attached to any activity")
            return
        }

        activityResultMap[REQ_START_SCAN] = AndroidScanResultHandler(result)
